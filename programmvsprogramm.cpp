...
#include"QMessageBox"
//#include"windows.h"
ProgrammVsProgramm::ProgrammVsProgramm()
{

}

void ProgrammVsProgramm::competiton()
{
...
void ProgrammVsProgramm::mouseReleaseEvent(QMouseEvent *ev)
{
    competiton();
    clearChess();
}

void ProgrammVsProgramm::AI(int *p, int *q, int side)
{
...
int ProgrammVsProgramm::positionValue(int row, int col, int side)
{
    ...
    while (n != 5) {
        ...
        k1 = chessType(n, row, col,side); n += 4;            /* k1,k2为2个反方向的棋型编号 */

        k2 = chessType(n, row, col,side); n -= 3;
...
int ProgrammVsProgramm::chessType(int n, int p, int q, int side)
{
    int COM=side;
    int MAN=side==1?2:1;
    int SPA=0;
...
void ProgrammVsProgramm::goStep(int n, int *i, int *j)
{
    switch (n) {
